...
function reverie_css() {
  if ( !is_admin() ) {
     ...
     wp_register_style( 'fonts',get_template_directory_uri() . '/webfont/styles.css', false );
...
foreach ($sidebars as $sidebar) {
	register_sidebar(array('name'=> $sidebar,
		...
		'after_widget' => '</div></article>',
		'before_title' => '<h6 class="footer"><strong>',
...
function reverie_entry_meta() {
	...
	echo '<time class="updated" datetime="'. get_the_time('c') .'" pubdate>'. sprintf(__('%s a las %s.', 'reverie'), get_the_time(' d/m/Y'), get_the_time()) .'</time>';
...
function ubermenu_searchform(){
	...
	<div class="row collapse"><label class="screen-reader-text" for="s">' . __('Search for:') . '</label>
		<div class="twelve columns">
		<div class="nine columns">
			<input type="text" value="' . get_search_query() . '" name="s" id="s" />
		</div>
		<div class="three columns">
			<input type="submit" id="searchsubmit" value="'. esc_attr__('Buscar') .'" class="postfix button" />
		</div>
		</div>
	</div>
	</form>';
...
if ( function_exists( 'add_image_size' ) ) { 
	add_image_size( 'homepage-destacado', 540, 9999 ); //300 pixels wide (and unlimited height)
}
